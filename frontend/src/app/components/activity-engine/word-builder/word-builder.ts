...
export class WordBuilderComponent implements OnInit, OnChanges {
  ...
  // Click-to-place Handlers (Alternative for mobile / ease of use)
  ...
  removeLetterFromBox(rowIdx: number, boxIdx: number, letterIdx: number): void {
    ...
    // Return the letter to the pool (mark isUsed as false for one matching unused instance in pool)
  ...
  clearRow(rowIdx: number): void {
    ...
    // Collect all letters placed in this row's boxes
